...
// 4.商品的模糊搜索
router.get('/v1/fuzzysearch',(req,res)=>{
  let kw=req.query.kw;
  let page=req.query.page;
  let offset=(page-1)*6;
  console.log(kw,page,offset)
  // 模糊查询关键字
  let sql="select count(pid) as count from kml_goods_product where title like  '%"+kw+"%' or subtitle like '%"+kw+"%' ";
  pool.query(sql,[kw],(err,results)=>{
    ...
    let pagesize=6;
    let pagecount=Math.ceil(rowCount/pagesize);
    let sql=`select * from kml_goods_product where title like '%${kw}%' or subtitle like '%${kw}%' LIMIT ${offset},${pagesize}`;
    pool.query(sql,[kw],(err,results)=>{
      if(err) throw err;
      console.log(pagesize,results)
      res.send({message:'查询成功',code:1,results:results,pagecount:pagecount,rowCount:rowCount});
    })
  })
})
